...
//---------------------------------------------------------------------
/*Преобразование Магнитной в Гринвическую (простое. только вектор)
Входные параметры:
		Ar1[][3] - радиус-вектор (x, y, z) [км]
Выходные параметры:
		Ar2[3] - радиус-вектор (x, y, z) [км]
Используемые процедуры:
		DegToRad
Подключаемые модули:
		<math.h>
...
//---------------------------------------------------------------------------
/*Преобразование Гринвической в Магнитную (простое. только вектор)
Входные параметры:
		Ar1[][3] - радиус-вектор (x, y, z) [км]
Выходные параметры:
		Ar2[3] - радиус-вектор (x, y, z) [км]
...
//---------------------------------------------------------------------------
/*Преобразование J2000 в Магнитную (простое. только вектор)
Входные параметры:
		Ar1[][3] - радиус-вектор (x, y, z) [км]
		t - время в юлиянских сутках
Выходные параметры:
...
//---------------------------------------------------------------------------
/*Преобразование Магнитной в J2000 (простое. только вектор)
Входные параметры:
		Ar1[][3] - радиус-вектор (x, y, z) [км]
		t - время в юлиянских сутках
Выходные параметры:
...
//---------------------------------------------------------------------------
/*Процедура перехода от J2000 к
эклиптической СК с центром в точке L2 и осью Х направленной на Солнце
Входные параметры:
		r[3] - радиус-вектор (x, y, z) [км]
		t - время в юлиянских сутках
Выходные параметры:
		r3[3] - радиус-вектор (x, y, z) [км]
Используемые процедуры:
		DegToRad
		PLEPH_R
		VectProizv
...
double  R_sol, N, S, mu1,
		EKL=DegToRad(23.4390346928518), //наклонение оси вращения Земли к плоскости эклиптики
		RL=1501000,   //расстояние до точки L2
	   r_sol[3], v_sol[3], n[3], r2[3]={0},r1[3]={0}, rl[3]={0}, rs[3]={0},
	   AM[3][3]={0};

	   AM[0][0]=1; AM[1][1]=AM[2][2]=cos(EKL); AM[1][2]=sin(EKL); AM[2][1]=-AM[1][2];
NCENTR=3;    //определяем центральное тело Земля - 3
NTARG=11;    //определяем небесное тело Солнце - 11

de405.calculateR(NTARG,NCENTR, t, r_sol); //определяем координаты Солнца
R_sol=norm(r_sol);

//переводим вектор КА и Солнца В эклиптическую геоцентрическую СК
for(int i=0; i<3; i++) {
	r1[i]=0;
	rs[i]=0;
	for(int j=0; j<3; j++){
...
for(int i=0; i<3; i++) rl[i]=-rs[i]/R_sol*RL;  //определяем положение точки L2 в эклиптической СК
for(int i=0; i<3; i++) r2[i]=r1[i]-rl[i];      //сдвигаем центр СК в точку L2
//Формируем матрицу поворота чтоб ось х была направленна на сонце
for(int i=0; i<3; i++) AM[0][i]=rs[i]/R_sol;
AM[2][0]=AM[2][1]=0; AM[2][2]=1;
VectProizv(AM[2], AM[0], AM[1]);
//получаем результирующий вектор по средством последнего поворота.
for(int i=0; i<3; i++){
	r3[i]=0;
	for(int j=0; j<3; j++)
		r3[i]+=AM[i][j]*r2[j];
...
/*Процедура перехода из плоскости эклиптики (X направлен на Солнце) в J2000
Входные параметры:
		r[3] - радиус-вектор (x, y, z) [км]
		v[3] - вектор скорости (vx, vy, vz) [км/с]
		t - время в юлиянских сутках
...
*/
   ...
   {
	  double 	EKL,    					//угол между эклиптикой и экватором
				RS,                         //расстояние до Солнца
				VS,                         //Скорость солнца
				WS,                         //угловая скорость вращения земли относительно солнца
				Ang,                        //угол между осями Х эклиптической иновой ск
				r_sol[3],                   //радиус-вектор солнца
				v_sol[3],                   //вектор скорости солнца
				r_sol_E[3],                 //вектор солнца в эклиптике
				v_per[3];					//учет переносной составляющей скорости
	  double 	MatrixX[3][3]={0},          //матрица поворота вокруг оси Х
				MatrixZ[3][3]={0},          //матрица поворота вокруг оси Z эклиптической системы координат
				MatrixPOV[3][3]={0},        //результирующая матрица поворота J2000->Ekl
				MatrixPOV_T[3][3]={0},      //результирующая матрица поворота Ekl->J2000
				MatrixPOVperen[3][3]={0},   //матрица учитывающая переносную скорость J2000->Ekl
				MatrixPOVperen_T[3][3]={0}; //матрица учитывающая переносную скорость Ekl->J2000
	  int 	NT=11,
			NC=3;
	  double r[3];
	  EKL = DegToRad(23.4390346928518);	//угол наклона земного экватора к
									//плоскости эклиптики на 01.01.2000
	  r[0]=r0[0]-1500000;
	  r[1]=r0[1];
	  r[2]=r0[2];
	  de405.calculateRV(NT, NC,t,r_sol,v_sol);

	  //найдём угловую скорость Солнца (и L2)
	  RS = norm(r_sol);
	  VS = norm(v_sol);
	  WS = VS/RS;

	  //Поворот вокруг оси X на угол EKL
	  ...
	  //Вектор Солнца в плоскости эклиптики
	  matr_X_vect(MatrixX, r_sol, r_sol_E);

	  //Угол между радиус-вектором Солнца и направлением на точку
	  //весеннего равноденствия [рад.]
	  Ang = atan2(r_sol_E[1]/RS, r_sol_E[0]/RS);

	  //Перевод вектора состояния КА в подвижную СК (поворот вокруг Z)
	  ...
	  //Вектор состояния КА в СК, центр которой находится в Земле,
	  //основная плоскость - плоскость эклиптики, ось X направлена на Солнце
	  matr_X_vect(MatrixPOV_T, r, r1);
	  matr_X_vect(MatrixPOV_T, v, v1);


	  //учёт переносной скорости
      ...
      ///Закоментрированно так как этот эффект учитывался раннее и алгоритм удобнее

	  //for(int i=0; i<3; i++)
		for(int i=0; i<3; i++) v1[i]+=v_per[i]*WS;
   }
//---------------------------------------------------------------------------
/*Процедура перехода из J2000 в плоскость эклиптики (X направлен на Солнце)
Входные параметры:
		r[3] - радиус-вектор (x, y, z) [км]
		v[3] - вектор скорости (vx, vy, vz) [км/с]
		t - время в юлиянских сутках
...
*/
   void J2000toEkl ( double r[3],double v[3], double t, double r1[3],double v1[3])
   {
	  ...
	  double 	MatrixX[3][3]={0},          //матрица поворота вокруг оси Х
				MatrixZ[3][3]={0},          //матрица поворота вокруг оси Z эклиптической системы координат
				MatrixPOV[3][3]={0},        //результирующая матрица поворота
				MatrixPOVperen[3][3]={0};   //матрица учитывающая переносную скорость
	  int 	NT=11,
			NC=3;
	 ...
	 // matr_X_vect(MatrixPOVperen, r, v_per);        это учтено в переходе к кси
	 // for(int i=0; i<3; i++)                        ВАЖНО!!!!!
	 //	for(int i=0; i<3; i++) v1[i]+=v_per[i]*WS;    оно нужно если нужен честный переход в эклиптику
   }
...
//---------------------------------------------------------------------
//процедура перевода из J2000 в Барицентрическую Солнечной Системы
...
//---------------------------------------------------------------------
//---------------------------------------------------------------------
//процедура перевода из Барицентрической Солнечной Системы в J2000
...
//---------------------------------------------------------------------
//---------------------------------------------------------------------
//процедура используемые при интегрировании магнитной линии при помощи пакета IGRF
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//переход от трех мерных координат к двухмерным определенных в плоскости перпендикулярной XY (экваториальной)
...
//---------------------------------------------------------------------------
//переход от двухмерных определенных в плоскости перпендикулярной XY (экваториальной) к  трех мерным координатам
